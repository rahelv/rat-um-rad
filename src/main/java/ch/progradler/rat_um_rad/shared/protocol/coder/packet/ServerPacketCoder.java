...
/**
 * Responsible for en-/decoding a {@link Packet<ServerCommand>}
 */
public class ServerPacketCoder extends PacketCoder<ServerCommand> {

    public ServerPacketCoder(PacketContentCoder contentCoder) {
        super(contentCoder);
    }

    public ServerPacketCoder(Coder<ChatMessage> messageCoder, Coder<UsernameChange> usernameChangeCoder, Coder<GameBase> gameBaseCoder, Coder<ClientGame> clientGameCoder, Coder<BuildRoadInfo> buildRoadInfoCoder) {
        super(messageCoder, usernameChangeCoder, gameBaseCoder, clientGameCoder, buildRoadInfoCoder);
    }
     ...
     */
    @Override
    public String encode(Packet<ServerCommand> packet, int level) {
    ...
    @Override
    public Packet<ServerCommand> decode(String encoded, int level) {
        ...
        ServerCommand command = ServerCommand.valueOf(fields.get(0));
        Object content = contentCoder.decodeContent(fields.get(1), contentType, level + 1);
        return new Packet.Server(command, content, contentType);
    }
}
